    ...
    <div class="topupbox1">
      <table class="table">
        ...
        <tbody>
          ...
          <tr>
            <td><input type="text" name="user_id" value="{{ $item->id}}"
                style="border:none;background-color:transparent;width:30px;" readonly></td>
            <td>{{$item->user->name}}</td>
            <td>{{$item->user_id}}</td>
            <td>{{ $item->reqamount}}</td>
            <td>{{$item->created_at}}</td>
            @if ($item->status ==0)
            <td>กำลังรอการยืนยัน</td>
            @endif
            @if ($item->status ==1)
            <td>ถูกปฏิเสธ</td>
            @endif
            ...
            @if($item->status==0)
            <td><a href="<?php echo " /admin/topup/deny/$item->id";?>" class="btn btn-danger"
                role="submit">ปฏิเสธคำขอ</a></td>
            <td><a href="<?php echo " /admin/topup/approve/$item->id";?>" class="btn btn-success ">ยืนยันคำขอ </a></td>
